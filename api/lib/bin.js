...
for (const file of files) {
    ...
    const tempDb = new Low(adapter, {});
    await tempDb.read();
    mergedData = { ...mergedData, ...tempDb.data };
}
// Set up database
const observer = new Observer(new JSONFile('in-memory-db.json'));
const db = new Low(observer, mergedData);
await db.read();
// Create app
            ...
            if (!writing) {
                ...
                db.write().catch((e) => {
                    if (e instanceof SyntaxError) {
                        return console.log(chalk.red(['', 'Error parsing files', e.message].join('\n')));
                    }
                    console.log(e);
                });
            }
